...
- **The SDK is `yourbot_sdk` (verified against 0.7.1; runs on 0.6.x too).** Renamed from `mmo_maid_sdk` in 0.6.0; the old name is a deprecation shim slated for removal. `requirements-dev.txt` tracks the deployed line (`>=0.7.1,<0.8`). The 0.6.x→0.7.x delta is additive (structured transport error codes, float `retry_after`, `respond(update_message=)`, keyword-safe `kv.increment` reorder) — see SDK-ASSUMPTIONS.md "Source diff (2026-06-17)". Production imports and the test stub both use `yourbot_sdk`; a stub-contract test blocks the legacy import from coming back and pins the 0.7.x exception `code` surface.
...
- Three slash commands: `/calc`, `/calc-config` (admin), `/calc-help`. All replies set `allowed_mentions: {"parse": []}`. Two components, both via `on_component(prefix=...)` with state encoded at the END of the `custom_id` (parse with `rsplit(":", 1)[-1]`; the prefixes contain `:`): (1) the "Show help" button on error embeds (`dch:help:<reason>` → `comp_show_help`), which **edits the error message in place** into the help card via 0.7's `respond(update_message=True)`; (2) the "Switch to degrees/radians" button on the `/calc-config` view (`dcc:angle:<target>` → `comp_set_angle`, admin re-checked on click). Both go through `_respond_update`, which falls back to a fresh ephemeral on pre-0.7 hosts.
...
- Current count: 333 tests, all green.
...
- **`log(x, 1)` is `DOMAIN_ERROR`, not `DIV_BY_ZERO`** (v0.2.9 / V1-03). `math.log(x, base)` divides by `log(base)` internally, so `base == 1` would raise `ZeroDivisionError` and surface as a misleading "make sure the second argument isn't zero" hint. `_i_log` now pre-checks `base == 1` and `base <= 0`, raising `ValueError` so the walker maps it to `DOMAIN_ERROR` with the base-aware diagnostic.
- **`enforce_total_cap` trims fields too, not just description** (v0.2.9 / V2-01). Two-pass: trim description first (the elastic field), then walk fields from the end, halving each value or dropping it if it can't shrink. Pre-v0.2.9 only the description was trimmed — a field-dominated embed could exceed Discord's 6000-char limit. `tests/test_embed.py:test_build_help_embed_always_under_cap` locks the help embed under the cap as `FUNCTIONS` grows.
...
- **Operators are an embed field, not a description line** (v0.2.11). Five inline fields in `/calc-help`: Operators + Basic + Roots/Exp/Log + Trig + Hyperbolic (Discord packs 3+2). Full-width rows: a `Commands` overview on top (v0.4.0), `Notes` below, and exactly one trailing config field — `Getting started` (numbered quick-start) when the server has never run `/calc-config` (`updated_at == 0`), else `Server settings` (live values). The Operators field is hand-rolled in `_build_help_payload` — it's not derived from `CATEGORY_ORDER` because operators aren't functions, they live in `parser._ALLOWED_BINOPS` / `_ALLOWED_UNARYOPS`. If you add an operator, manually add a line to the Operators field value. If you add a slash command, add it to the `Commands` field too — `test_help_payload_lists_every_command` enforces it.
